...
class Solver:
    def __init__(self, ni, nh, no,lr):
        ...
        self.iweight = makeMatrix(self.ninput, self.nhidden)
        self.oweight = makeMatrix(self.nhidden, self.noutput)
...
## Implementing Feedforward network
    ...
    def tests(self,test_data):    
        ...
        for i in range(0,len(test_data)):
            ...
            Matrix[s/90][t/90]+=1
            orient=test_data[i][0]+" "+test_data[i][1]+"\n"
            fl.write(orient)
            if t==s:
                count+=1
   ...
   # Finding the nearest neighbour of the data point and return the orientation  
    ...
    def knn(self,train_data,test_data,k):
        ...
        for i in range(0,len(test_data)):
            t=list(test_data[i])
            orient=self.knnd(train_data,t,k)
            if orient == t[1]:
                count+=1  
            Matrix[int(t[1])/90][int(orient)/90]+=1       
            orient=t[0]+" "+orient+"\n"
            fl.write(orient)
