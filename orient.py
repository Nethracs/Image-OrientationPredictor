...
# Read in the file #
if len(sys.argv) != 5:
    print "Usage: python  orient.py train-data.txt test-data.txt knn k"
    sys.exit()
(train_file, test_file) = sys.argv[1:3]

print "Learning images..."

solver= Solver(0,0,0,0)
...
if(sys.argv[3]=="knn"):
    ...
    solver.knn(train_data,test_data,sys.argv[4])
    print ("----Seconds----",time.time()-start_time) 
if (sys.argv[3]=="nnet"):
    start_time=time.time()
    solver= Solver(192,sys.argv[4],4,0.7)
    solver.train(train_data)
    solver.tests(test_data)
    print ("----Seconds----",time.time()-start_time)  
if (sys.argv[3]=="best"):
    with open("bestmodel.txt", "r") as ins:
        for line in ins:
            ...
            if a[0]=="knn":
                ...
                solver= Solver(0,0,0,0)
                start_time=time.time()
                solver.knn(train_data,test_data,a[1])
                print ("----Seconds----",time.time()-start_time) 
